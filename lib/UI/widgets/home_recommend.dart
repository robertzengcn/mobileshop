    ...
    return BlocBuilder<FeaturedsBloc, FeaturedState>(
        builder: (context, state) {
          ...
          if(state is Featuredloaded){
           ...
           developer.log('feature list all load', name: 'my.app.category', error: featurelist);
...
//                  color: Color(0xFF6991C7),
                ...
                child:Row(
                    children:getWidgets(featurelist)
                )
            );
          }
          return Center(child: CircularProgressIndicator());
  ...
  List<Widget> getWidgets(List<Product> lstFeatureds) {
   ...
   if(plength==0){
    ...
    developer.log('feature list render', name: 'my.app.category', error: plength);
   int rownumbe=(plength/2).round();
   for (int i = 0; i < rownumbe-1; i++) {
     ...
     widgetlist.add(
       ...
       Expanded(
       flex:5,
         child:ProductCard(productDetails:lstFeatureds[i]),
       ),
           Expanded(
         flex:5,
         child:ProductCard(productDetails:lstFeatureds[i+1])
     )
                  ]
              )
           ]
